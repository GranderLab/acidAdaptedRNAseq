...
#' default x = 1 cpm and n is the number in smallest replicate #### recommended by Gordon Smyth (edgeR)
...
.contrasts.limma<-function(targets, design){
	...
	for (i in samples.cont) { 
		...
		for (i in samples.cont) {
			...
			contrast_group=paste(contrast_group,contrast,sep=" ") #one string
	}}}
	
	contrasts=strsplit(contrast_group, split=" ", fixed = T, perl = FALSE, useBytes = FALSE)
	contrasts=unlist(contrasts)
	conts=grep("-", contrasts, value=T)
